#!/bin/bash
# Task 14C-0: MySQL dev smoke test
# Tests core business chain on real MySQL database
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/work-orders?pageNo=1&pageSize=1")
check "Work order list (sequence available)" "SUCCESS" "$r"

echo ""
echo "=== Step 2: Create third-party part ==="
r=$(curl -s -X POST -H "$H1" -H "$H2" -H "$CT" \
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/parts?pageNo=1&pageSize=10")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/inventory/stocks/$PART_ID")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/work-orders/$WORK_ORDER_ID")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/inventory/flows?partId=$PART_ID&flowType=RESERVE&pageNo=1&pageSize=10")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/work-orders/$WORK_ORDER_ID/payment-summary")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/inventory/flows?partId=$PART_ID&flowType=CONSUME&pageNo=1&pageSize=10")
...
r=$(curl -s -X POST -H "$H1" -H "$H2" -H "$CT" \
  ...
  "$BASE/api/admin/parts/third-party")
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/parts?pageNo=1&pageSize=20")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/inventory/stocks/$PART2_ID")
...
r=$(curl -s -H "$H1" -H "$H2" "$BASE/api/admin/inventory/flows?partId=$PART2_ID&flowType=RELEASE&pageNo=1&pageSize=10")
