...
> 当前系统已从单门店 MVP 演进为“门店售后库存管理系统”：以门店为数据隔离边界，支持超管全局管理、门店管理员管理本门店，覆盖员工小程序、管理端、维修工单、配件库存、支付退款、官方结算、报销台账、财务报表、资方台账和 Excel 导出。
...
- 客户侧小程序预约排号系统
- 客户到店预约、签到、排队、服务进度查询
- 员工端今日预约队列、叫号、接待、预约转工单草稿
- 客户侧建议作为独立 Maven 模块设计，详见 `docs/CLIENT_APPOINTMENT_MODULE_ANALYSIS.md`
...
export            Excel 导出
funding           资方台账
appointment       客户预约、到店签到、排号、接待（规划）
client            客户身份、客户侧接口（规划）
...
9. 小程序体验完善
  ...
  - M9B: 旧版 Staff 结算工单已被 M24B 状态机替代。当前库存正式扣减发生在“标记维修完成”，交付关闭只完成业务交付，不再次改变库存。
  ...
  - M12C: admin-web 报销台账真实接口对接完成。列表/详情/确认/驳回均对接后端 API，PENDING 显示操作按钮，已终态隐匿；确认校验金额>0，驳回校验原因必填。
    ...
    - M14B: admin-web Excel 导出下载对接完成。财务日报/月报/范围导出与报销台账导出均走后端真实 `.xlsx` 文件流，支持文件名解析、loading 和失败提示。
    - M14C: Excel 导出端到端断点检查完成。后端 API、admin-web 参数映射、Blob 下载、错误提示、空数据导出和业务只读边界均已核对。
    - M18-M29: 已补齐客户/车辆搜索、微信登录绑定、多门店账号边界、M24B 清洁状态机、员工与权限、资方台账、管理端与员工小程序生产验收修复。
10. 真实门店试运行
11. 客户侧预约排号系统规划
  - 客户侧不直接创建正式工单，不直接预占库存，不直接收款退款。
  - 推荐链路：客户预约 / 到店签到 / 排号 → 员工接待 → 转工单 DRAFT → 员工提交后进入维修与库存预占。
...
- `docs/CLIENT_APPOINTMENT_MODULE_ANALYSIS.md`：客户侧预约排号系统业务与技术分析
