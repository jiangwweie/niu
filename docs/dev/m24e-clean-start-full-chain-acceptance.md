...
|------|------|----------|
| 1. 创建工单 | 页面显示“新建中” | ✓ |
| 2. 提交工单 | 页面显示“维修中”，库存显示“已预占” | ✓ |
| 3. 部分收款 | 不改变进度/库存 | ✓ |
| 4. 标记维修完成 | 页面显示“维修完成”，库存显示“已扣减” | ✓ Toast: "已标记维修完成，库存已扣减。" |
| 5. 未收齐时交付 | 阻止，提示"请先收齐尾款后再交付关闭" | ✓ |
| 6. 补齐尾款 | 页面显示“已收齐” | ✓ |
| 7. 交付关闭 | 页面显示“已交付”，不产生库存流水 | ✓ 确认文案: "库存已在标记维修完成时扣减，本操作不再改变库存。" |

## 7. 无需收款链路验证（代码审查）

| 步骤 | 预期 | 代码实现 |
|------|------|----------|
| 1. 应收为 0 时标记维修完成 | 必须填写“无需收款原因” | ✓ 弹窗展示原因选择器 |
| 2. 不填原因 | 阻止提交，提示"应收为0时必须选择无需收款原因" | ✓ |
| 3. 选择原因后维修完成 | 页面显示“维修完成”和“无需收款” | ✓ |
...
| 步骤 | 预期 | 代码实现 |
|------|------|----------|
| 1. 维修中取消 | 页面显示“已取消”，库存显示“已释放” | ✓ 取消弹窗警告"工单取消后将自动释放预占库存" |
| 2. 有净实收时取消 | 页面显示“待退款” | ✓ |
...
- 需在运行环境中验证
- 官方结算与客户实收在详情页中分区展示（官方订单号、官方结算状态、官方结算金额）

## 10. 小程序验收（代码审查）

### 10.1 不存在的状态

小程序不展示旧工单状态文案，相关旧状态统一显示为“旧状态，请先清理试运行数据”。
...
- 不说"交付时扣库存" ✓
- 已收齐场景："工单已结清。确认交付关闭后，工单将进入已交付状态；库存已在标记维修完成时扣减，本操作不再改变库存。"
- 无需收款场景："无需收款工单（原因：{reason}）。确认交付关闭后..."
...
- [x] 工单状态机正确：新建中 → 维修中 → 维修完成 → 已交付 / 已取消
