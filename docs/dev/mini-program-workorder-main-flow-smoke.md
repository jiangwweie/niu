...
3. **记录支付**：已在工单详情页 `work-order-detail` 补齐对应 Action Bar 的 `onPayment` 事件和弹窗 UI。点击确认后调用 `POST /api/staff/work-orders/{id}/payments`。支付不会自动结算工单。
4. **结算工单**：已在详情页补齐弹窗 UI。确认后调用 `POST /api/staff/work-orders/{id}/settle`。
...
3. 进入该新工单详情，点击“记录支付”，录入 100 元现金。弹窗关闭后，上方“实收金额”实时刷新为 `¥100`。
4. 点击“结算工单”。若费用达标，系统正常扣减库存并在页面标记“SETTLED”。如果实收金额不足，后端返回 400，前端 Toast“报错内容：xxx”，状态依然保留在 `PENDING_SETTLEMENT` 等阶段。
5. 后退返回列表时，通过 `onShow` 的生命周期拉取最新列表，同步刷新卡片状态。

## 5. 类型与编译结果
- `npx tsc --noEmit` 报告 39 个错误，全部为 tdesign/miniprogram 类型定义和 placeholder import 的历史问题，本次未新增。
