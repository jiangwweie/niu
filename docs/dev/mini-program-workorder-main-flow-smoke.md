...
3. **记录支付**：已在工单详情页 `work-order-detail` 补齐对应 Action Bar 的 `openPaymentPopup` 事件和 `t-popup` 弹窗 UI。底部按钮只打开弹窗，点击“确认记录”后调用 `POST /api/staff/work-orders/{id}/payments`。支付不会自动结算工单。
4. **结算工单**：已在详情页补齐 `t-dialog` 确认弹窗 UI。底部按钮只打开确认弹窗，点击确认后调用 `POST /api/staff/work-orders/{id}/settle`。
...
- **记录支付**：仅对非 CANCELLED、非 SETTLED 状态显示。结算后不再允许补录收款。
...
- Request wrapper / auth / config 均原样保留，未做任何更改。
...
3. 进入该新工单详情，默认不直接显示支付/退款表单；点击“记录支付”，底部 `t-popup` 正常弹出。不填金额点击确认时，Toast 正常提示且不报 selector 错误。
4. 录入 100 元现金并确认。接口成功后弹窗关闭，上方“实收金额”实时刷新为 `¥100`。
5. 点击“结算工单”，`t-dialog` 确认弹窗正常显示。若费用达标，系统正常扣减库存并在页面标记“SETTLED”。如果实收金额不足，后端返回 400，前端 Toast“报错内容：xxx”，状态依然保留在 `PENDING_SETTLEMENT` 等阶段。
6. 后退返回列表时，通过 `onShow` 的生命周期拉取最新列表，同步刷新卡片状态。
